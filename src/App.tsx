...
function Admin() {
  ...
  const createCalendar = () => { const name = window.prompt('Kalendernavn'); if (name?.trim()) addCalendar({ name: name.trim(), owner: name.trim(), color: 'orange', active: true, source: 'ICS' }) }
  const editCalendar = (id: string, name: string) => { const nextName = window.prompt('Nyt navn', name); if (nextName?.trim()) updateCalendar(id, { name: nextName.trim() }) }
