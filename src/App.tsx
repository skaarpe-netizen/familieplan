...
function MealPanel() { const { meals, isLoading, isError, error } = useMealData(); const today = new Date(); const date = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`; const meal = meals.find((item) => item.date === date); return <section className="panel meal-panel"><div className="panel-header"><div><p className="eyebrow">I dag</p><h2>Madplan</h2></div><CookingPot size={30} className="meal-icon" /></div>{isLoading ? <div className="empty-widget"><p>Henter madplan...</p></div> : isError ? <div className="empty-widget"><CookingPot size={26} /><p>Madplan kunne ikke hentes</p><small>{error instanceof Error ? error.message : 'Kontrollér meal_events og RLS'}</small></div> : meal ? <div className="meal-content"><div className="meal-image">🍽</div><div><span className="meal-label">Aftensmad</span><h3>{meal.title}</h3><p>Dagens ret fra familiens madkalender</p></div></div> : <div className="empty-widget"><CookingPot size={26} /><p>Ingen ret planlagt i dag</p><small>{meals.length ? `${meals.length} retter er hentet, men ingen er dateret i dag` : 'Ingen retter er synkroniseret endnu'}</small></div>}</section> }
