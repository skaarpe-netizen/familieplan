...
function Admin() {
  ...
  const editCalendar = async (id: string, name: string) => { const nextName = window.prompt('Nyt navn', name); if (!nextName?.trim()) return; try { await updateCalendarRecord(id, { name: nextName.trim() }); updateCalendar(id, { name: nextName.trim() }); await queryClient.invalidateQueries({ queryKey: ['calendar-data'] }); } catch { window.alert('Kalenderen kunne ikke opdateres') } }
