...
import { saveMealSettings } from './services/mealService'
import { syncMealPlan } from './services/mealSyncService'
...
function WeatherPanel() {
  ...
  const [current, setCurrent] = useState<{ temperature: number; wind: number; rain: number; label: string } | null>(null)
  useEffect(() => {
  ...
  const clothing = current ? (current.temperature < 8 ? 'Vinterjakke' : current.temperature < 16 ? 'Tynd jakke' : 'T-shirt og let trøje') : 'Afventer vejrdata'
  return <section className="panel weather-panel"><div className="panel-header"><div><p className="eyebrow">Hjemme hos os</p><h2>Vejr</h2></div><CloudSun size={34} className="weather-icon" /></div><div className="weather-main"><div><span className="temperature">{current ? `${current.temperature}°` : '--'}</span><span className="condition">{current?.label ?? 'Ingen vejrdata endnu'}</span></div><div className="weather-place">{weather.city || 'Ingen lokation'}<br /><small>Open-Meteo</small></div></div><div className="weather-stats"><span><Wind size={17} /> {current ? `${current.wind} km/t` : '--'}</span><span><ThermometerSun size={17} /> {current ? `Føles som ${current.temperature - 1}°` : '--'}</span><span><CloudSun size={17} /> {current ? `${current.rain}% regn` : '--'}</span></div><div className="clothing"><span className="clothing-icon">◒</span><div><strong>{clothing}</strong><small>{current ? (current.temperature < 16 ? 'Det bliver køligt i aften' : 'En lun dag forude') : 'Indstil vejr under Administration'}</small></div></div></section>
}

function MealPanel() { const { meals, isLoading, isError, error } = useMealData(); const [syncing, setSyncing] = useState(false); const [status, setStatus] = useState(''); const queryClient = useQueryClient(); const today = new Date(); const date = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`; const meal = meals.find((item) => item.date === date); const sync = async () => { setSyncing(true); setStatus('Synkroniserer...'); try { const result = await syncMealPlan(); await queryClient.invalidateQueries({ queryKey: ['meal-data'] }); setStatus(`${result.synced ?? 0} retter synkroniseret`) } catch (syncError) { setStatus(syncError instanceof Error ? syncError.message : 'Madplans-sync fejlede') } finally { setSyncing(false) } }; return <section className="panel meal-panel"><div className="panel-header"><div><p className="eyebrow">I dag</p><h2>Madplan</h2></div><CookingPot size={30} className="meal-icon" /></div>{isLoading ? <div className="empty-widget"><p>Henter madplan...</p></div> : isError ? <div className="empty-widget"><CookingPot size={26} /><p>Madplan kunne ikke hentes</p><small>{error instanceof Error ? error.message : 'Kontrollér meal_events og RLS'}</small></div> : meal ? <div className="meal-content"><div className="meal-image">🍽</div><div><span className="meal-label">Aftensmad</span><h3>{meal.title}</h3><p>Dagens ret fra familiens madkalender</p></div></div> : <div className="empty-widget"><CookingPot size={26} /><p>Ingen ret planlagt i dag</p><small>{meals.length ? `${meals.length} retter er hentet, men ingen er dateret i dag` : 'Ingen retter er synkroniseret endnu'}</small></div>}<button className="text-action" onClick={sync} disabled={syncing}><RefreshCw size={15} className={syncing ? 'spin' : ''} /> {syncing ? 'Synkroniserer...' : 'Synkroniser madplan'}</button>{status && <small className="sync-status">{status}</small>}</section> }
...
function MessagesPanel() {
  ...
  return <section className="panel messages-panel"><div className="panel-header"><div><p className="eyebrow">Fra familien</p><h2>Beskeder</h2></div><MessageCircle size={29} className="message-icon" /></div>{messages.length ? <div className="message-list">{messages.slice(0, 2).map((message) => <div className="message-item" key={message.id}><span className="avatar">{message.author[0]}</span><div><strong>{message.text}</strong><small>{message.author} · {message.createdAt}</small></div></div>)}</div> : <div className="empty-widget"><MessageCircle size={26} /><p>Ingen beskeder endnu</p><small>Skriv den første besked til familien</small></div>}<button className="text-action" onClick={writeMessage}>Skriv en besked <Plus size={16} /></button></section>
}
...
function Admin() {
  ...
  const [mealSyncing, setMealSyncing] = useState(false)
  ...
  const setLocalMealUrl = useFamilyStore((state) => state.setMealUrl)
  const setMealUrl = (value: string) => { setLocalMealUrl(value); if (/^https?:\/\//i.test(value.trim())) void saveMealSettings(value.trim()).catch(() => undefined) }
  ...
  const editCalendar = async (calendar: typeof calendars[number]) => { const nextName = window.prompt('Kalendernavn', calendar.name); if (!nextName?.trim()) return; const nextUrl = window.prompt('ICS kalenderlink', calendar.icsUrl ?? ''); if (!nextUrl?.trim() || !/^https?:\/\//i.test(nextUrl.trim())) { window.alert('Indtast et gyldigt ICS-link, der starter med http:// eller https://'); return } try { await updateCalendarRecord(calendar.id, { name: nextName.trim(), icsUrl: nextUrl.trim() }); updateCalendar(calendar.id, { name: nextName.trim(), icsUrl: nextUrl.trim() }); await queryClient.invalidateQueries({ queryKey: ['calendar-data'] }); } catch (error) { window.alert(error instanceof Error ? `Kalenderen kunne ikke opdateres: ${error.message}` : 'Kalenderen kunne ikke opdateres') } }
  ...
  const saveMealPlan = async () => { if (!mealUrl.trim() || !/^https?:\/\//i.test(mealUrl.trim())) { setMealStatus('Indtast et gyldigt ICS-link'); return } try { await saveMealSettings(mealUrl.trim()); setMealStatus('Madplan gemt'); } catch (error) { setMealStatus(error instanceof Error ? error.message : 'Madplanen kunne ikke gemmes') } }
  const runMealSync = async () => { setMealSyncing(true); setMealStatus('Synkroniserer...'); try { const result = await syncMealPlan(); await queryClient.invalidateQueries({ queryKey: ['meal-data'] }); setMealStatus(`${result.synced ?? 0} retter synkroniseret`) } catch (error) { setMealStatus(error instanceof Error ? error.message : 'Madplans-sync fejlede') } finally { setMealSyncing(false) } }
  useEffect(() => { if (!/^https?:\/\//i.test(mealUrl.trim())) return; const timer = window.setTimeout(() => void runMealSync(), 800); return () => window.clearTimeout(timer) }, [mealUrl])
